...
Item {
  ...
  // Custom bar plugins are created by a Loader and configured immediately
  // afterwards by shell.qml. These cannot be `required`: Loader creation
  // happens before the host can inject them, which prevents the bar from
  // loading altogether.
  ...
  component BarPanel: PanelWindow {
    ...
    margins {
      top: root.barHidden && root.position === "top" ? -root.barSize : (root.position === "top" ? Style.space(6) : 0)
      bottom: root.barHidden && root.position === "bottom" ? -root.barSize : (root.position === "bottom" ? Style.space(6) : 0)
    ...
    // Keep the horizontal panel full-width so centered modules stay truly
    // centered on every output, but render its contents inside a compact
    // floating surface.
    implicitWidth: root.vertical ? root.barSize : 0
    implicitHeight: root.vertical ? 0 : root.barSize + Style.space(4)
    ...
    Component {
      ...
      Item {
        ...
        readonly property real cardWidth: Math.min(800, Math.max(0, width - Style.space(12)))
        readonly property real cardHeight: root.barSize
        readonly property real cardX: Math.round((width - cardWidth) / 2)
        readonly property real cardY: Math.round((height - cardHeight) / 2)
        ...
        Rectangle {
          ...
          color: root.transparent
            ? "transparent"
            : Qt.rgba(root.background.r, root.background.g, root.background.b, 0.96)
          border.width: root.transparent ? 0 : 2
          border.color: Color.accent
        }

        CenterModules {
          anchors.fill: barCard
          anchors.leftMargin: Style.space(8)
          anchors.rightMargin: Style.space(8)
        }

        LeftModules {
          anchors.left: barCard.left
          anchors.leftMargin: Style.space(6)
          anchors.verticalCenter: barCard.verticalCenter
        }

        RightModules {
          anchors.right: barCard.right
  ...
  component ModuleSlot: Item {
    ...
    // Preserve the stock widget proportions: the floating card's height is
    // exactly the bar height, so icons and labels fit without scaling.
    readonly property real moduleScale: 1
    ...
    function animateClick() {
      clickScale = 0.92
      clickReleaseTimer.restart()
    }

    Timer {
      id: clickReleaseTimer
      interval: 75
      onTriggered: slot.clickScale = 1
    }
